...
| `--primary` (유일한 커스텀 색) | **딥 그린 확정 (2026-08-20, SCR-003 카드 실물 위 사용자 판단)** — 라이트 `oklch(0.5 0.15 150)` / 다크 `oklch(0.48 0.14 150)`, foreground `oklch(0.98 0.016 73.684)`. 대비 실측(**버튼 조합 = primary 배경 + foreground 글자** 기준): 라이트 **5.22:1**, 다크 **5.73:1** (SC 1.4.3 4.5:1 통과). 사용자가 고른 원안은 `oklch(0.55 0.15 150)`였으나 같은 foreground에 **4.27:1로 미달**이라 명도만 −0.05 조정. 탈락: 형광 `oklch(0.70 0.19 45)` 2.7:1(비텍스트 강조 전용으로만 가능) · shadcn orange(경고색으로 읽혀 등급 배지와 의미 충돌) · 잉크 블루(zinc 대시보드 클리셰와 같은 방향) |
| `--primary-text` (텍스트 전용, 2026-09-02 신설) | 라이트 `oklch(0.5 0.15 150)`(= primary와 동일) / 다크 `oklch(0.65 0.14 150)`. **위 `--primary` 실측은 버튼 조합 기준이었고 `text-primary`로 쓸 때는 검증된 적이 없었다** — Day 17 접근성 감사에서 `/how` GitHub 링크가 다크 배경 위 **3.24:1**로 적발(SC 1.4.3 위반). 배경용은 어두워야 하고 텍스트용은 밝아야 해 **한 토큰으로 동시 만족이 불가능**하다(실측: L 0.60이면 텍스트는 5.34로 통과하지만 버튼 흰 글자가 3.48로 깨진다). 그래서 용도를 쪼갰다. 다크 0.65 실측 — 배경 위 **6.45:1**, 카드 위 **5.71:1**. 사용처 3곳: `/how` 링크 · 복사 완료 알림 · 공유 완료 알림. ⚠️ **배경색으로 쓰지 말 것** (그건 `--primary`) |
