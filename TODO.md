...
- **Day 9(8/26 수) 저녁 = 스코프 컷 판정 (캘린더 등록됨)** — Day 8 전 항목 완료로 컷 대상 없음이 기본값. 판정 시 함께: ① 게릴라 테스터→본인 테스트 전환 PLAN 반영(make-plan) ② 실기기 공백 대체안 PLAN 문구(Day 6 인계 잔여) ③ **이슈 #1 — 카카오톡이 긴 공유 URL(해시 ~1.3KB)을 스크랩하지 않아 미리보기 미표시** (8/25 실사용 확인, 서버 정상 — 대응 선택지는 이슈 참조)
