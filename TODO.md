...
- [ ] **EOD 기록 세션 (8/26 저녁)** — Obsidian TIL · Notion 당일 등록

### 사용자 담당

- [x] **CTA 육안 검증** (localhost:3001) — link 진입 시 "내 성적표도 뽑아보기" 노출 / cli 진입 시 "내일 또 뽑아보세요" 노출, 양쪽 정상
- [ ] **판정 ① 스코프 컷 대상** — Day 8·9 전 항목 완료로 **컷 없음이 기본값**
- [ ] **판정 ② 이슈 #1 대응 방향** — B(해시 없는 공유 링크)의 Day 10~12 이연 확정 여부
- [ ] 배포 URL 육안 확인 (`scored.kr` — 줄바꿈 + 유입 CTA)
- [ ] (Day 8 이월) `npx scored` 실행해 카드 저장·링크 복사 직접 체험
- [ ] (보류 유지) npm publish `scored` 시점

### Day 9 → Day 10 인계

- **미이행 판정 2건 — Day 10 시작 시 처리**: ③ 게릴라 테스터→본인 직접 테스트 전환 PLAN 반영 ④ 실기기 공백 대체안 PLAN 문구(Day 6 인계 잔여). **둘 다 make-plan 경유 필수** (운용 원칙 4번 — 지정 스킬 없이 직접 편집 금지)
