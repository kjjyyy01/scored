...
> 일정 (2026-08-22 재조정 — 8/19 서밋 공백 + **8/21~8/22 작업 공백 2일**): Day 0 = 8/11(화), Day 1~2 = 8/12~13, Day 3 = 8/14~8/16(일), Day 4 = 8/18(화), Day 5 = 8/20(목) → **Day N≥6 = 8/17+N: Day 6 = 8/23(일)**, Day 9 = 8/26(수), Day 14 = 8/31(월), Day 19 = 9/5(토), 9/6~9/7 버퍼 잔량, **9/8~9/9 사용자 직접 전 플로우 완주**, **Day 20 런칭 = 9/10(목)**, Day 21 = 9/11(금) 버퍼, 판정일 **9/24(목)** — 6차 조정 2026-09-08(확장 +2일, 지렛대 미적용, 버퍼 잔량 불변)
...
- [x] ~~🔒 **Day 21 초기 유저 인터뷰를 런칭 직후(Day 20~21)로 앞당겨 실시**~~ → ⛔ **폐지 2026-09-08 (사용자 결정)**. 이 줄의 경고가 그대로 실현된다 — 판정일(9/24) 전에 외부 관찰을 회수하지 못하므로 **킬 크라이테리아 해석에 근거가 없는 상태로 판정에 들어간다.** 외부 검증 0 확정. 되살아나는 조건은 PLAN §판정 해석 규칙 ⓐ(지표 미달 시 "피벗"을 선택하려는 경우)뿐이며, 질문지는 `docs/인터뷰.md`에 보존
...
- [ ] 🆕 **브랜드 자산 보완 — 게시 전 필수 (2026-09-08 추가, 미이행 회수)**. 상세·범위 고정은 `docs/PLAN.md` 진행 체크리스트 참조
  - [ ] favicon 교체 — `src/app/favicon.ico`가 **`create-next-app` 기본값 그대로**(MD5 대조 확인, 8/11 스캐폴드 생성분)
  - [ ] `icon`·`apple-icon` 신설 (현재 둘 다 없음)
  - [ ] 메인 로고/워드마크 (현재 텍스트 `scored`뿐)
  - [ ] `manifest` 필요 여부 판단 (PWA 스코프 밖 → 필수 아님)
  - [ ] `public/{file,globe,next,vercel,window}.svg` 5개 삭제 — **코드 참조 0건 확인 완료**, 배포물에 Vercel·Next 로고 포함 중
  - [ ] 로고 확정 시 `src/app/opengraph-image.tsx`(정적 OG) 정합 확인
  - ⚠️ **왜 게시 전인가**: 게시 후 교체하면 이미 공유된 카드·북마크·검색 결과에 남의 로고가 남는다
...
- [ ] `/review-animations` 직접 실행 (스킬이 사용자 호출 전용) — ⚠️ **미실행으로 종결**(2026-09-09 동기화 시 확인). 축 패스 완료 조건은 위 육안 판정으로 충족됐고 Day 12 이후 모션 변경이 없어 게이트를 막지 않았다. 되살릴 조건: 모션을 다시 손대는 시점
- [x] 판정 통과 후 main 머지·배포 승인 — 반영 확인(중앙 스테이지 `md:max-w-md`·`scroll-behavior` 둘 다 main 실재)
...
> ⚠️ **동기화 공백 (2026-09-09 회수)**: 아래 Day 13~20은 **작업 당시 TODO.md에 반영되지 않았다**. 실작업은 진행됐고 기록은 `docs/PLAN.md` 진행 체크리스트와 `docs/history.md`에 남았으나, 이 파일만 Day 12에서 멈춰 있었다. 상시 규칙 *"매일 EOD 기록 블록 / TODO.md 갱신"*의 후자가 20일간 미이행이었다. 아래는 그 두 문서를 출처로 **사후 재구성**한 것이며, 각 항목의 완료 판정 근거는 원 문서에 있다.

## Day 13 (**9/3 수** — 예정 8/30에서 밀려 런칭 5일 전 회수) — 검증 세션

> 예정일이 Day 12에 잠식돼 결번이었던 것을 회수. PLAN 실패 조건 9 대상 구간.

### Claude 담당

- [x] 검증 세션 진행 보조 + 결과 기록 (`docs/PLAN.md`·`backlog.md` 반영)
- [x] 자문 ③ 보완 제안 2건 처리 — CLI 출력 서식은 `backlog.md` 개선행(의존성 0 제약·재발행 비용 명시), 계정·기록 축적·랭킹은 **무업로드 원칙·재미 장치 1개 규칙과 충돌**해 backlog 근거 보강으로 종결

### 사용자 담당

- [x] **본인 직접 테스트 완주** — 시크릿 창·새 프로필에서 `npx @jong-yeon/scored`부터 공유까지 전 플로우. **DoD 잔여 1건(CLI 수동 완주) 해소 → 14/16 → 15/16**. 실측 카드로 BR-009 커브 역산 일치 확인(51점 = B+)
- [x] 자문 4문항 중 ③ *"다시 올 이유가 화면에 있나"* — **"없다"**
- [ ] ⚠️ **자문 ①②④ 미회수** — 특히 ④ *"어느 순간 웃었나"* 는 PLAN이 **"본인 테스트의 최대 사각지대"**로 지목한 항목이다. 외부 검증 0이 확정된 상태라 회수 기회는 9/8~9/9 완주뿐
- [x] ~~외부 눈 1명~~ — **0명. 실패 조건 9 발동 상태 유지**(2026-08-27 폐지 결정)

## Day 14 (**8/31 일**) — 3축 리뷰 + 코드 성숙도 체크

### Claude 담당

- [x] 3축 리뷰 — `code-review`(정확성) · `/ponytail-review`(복잡도) · mattpocock `/code-review`(스펙 준수)
- [x] 코드 성숙도 체크리스트 통과 — 네이밍·중복·타입 안전성·에러 처리·DESIGN.md 대조·반응형·ARCHITECTURE.md 일치
- [x] **보안 4종** — 클라이언트 번들 시크릿 0 / DB 없음(RLS 비해당) / 입력 검증 / `npm audit` + git 이력 시크릿 확인
- [x] 재미 장치 확률·판정 로직 테스트 통과 확인
- [x] 반응형 실사 — md+ 2열 `328px 328px` / md 미만 1열, 가로 스크롤 없음

## Day 15~16 (**9/1~9/2**) — 출시 준비: 관측·법적 고지·장애 대비

> **Day 16 잔여 0** (2026-09-02 종결). 피드백 채널은 2026-08-31 사용자 결정으로 스코프 제외 → `backlog.md` 개선행.

### Claude 담당

- [x] **Sentry 연동 + 알림 채널 실설정** — 2026-09-02 도달 확인. environment `vercel-production` 규명
- [x] **GA4 이벤트 실발화 검수** — DebugView 대신 **실시간 보고서로 갈음**, 네트워크 레벨 EVT 9종 전수
- [x] **핵심 플로우 E2E 1개** — 핵심 1 + 메타데이터 5
- [x] **분석 도구 사용 고지 페이지** — `/how`
- [x] **OG 이미지** — 정적 + 동적 둘 다
- [x] **장애 대비 3종** — `RUNBOOK.md`(롤백 절차·백업 정책·장애 시 행동 3줄) + **롤백 리허설 실시**(2026-09-02, 4단계 정상)
- [x] 🐛 롤백이 남긴 **자동 도메인 할당 OFF 상태 복구** — 리허설 부작용을 당일 회수하고 함정을 RUNBOOK에 고정
- [x] ~~피드백 채널 심기~~ → **스코프 제외**(2026-08-31 사용자 결정). ⚠️ 대가: 제품에 정성 회수구 0개

### 사용자 담당

- [x] Sentry 알림 수신 확인(이메일)
- [x] 피드백 채널 스코프 제외 결정 (2026-08-31)

## Day 17 (**9/2 화, 하루 앞당겨 처리**) — 출시 전 기술 점검

> **Day 17 잔여 0**. iOS Safari만 실기기 아닌 부분 검증.

### Claude 담당

- [x] 사이트맵 + **Search Console 도메인 속성 등록** — 가비아 DNS TXT, 공개 DNS 8.8.8.8 전파 확인. `sitemap.xml` 제출 200, URL 2건(`/`·`/how`) — `/report`는 9/1 결정대로 의도적 제외
- [x] JSON-LD (랜딩 `SoftwareApplication`) + canonical 4라우트
- [x] **Lighthouse 접근성·SEO·Best Practices 전 화면 100**
- [x] **접근성 스팟 패스** — `/how` 다크 대비 결함 1건 수정(3.24:1 → 6.45:1). 원인이 링크 하나가 아니라 **토큰 용도 충돌**이라 텍스트 전용 `--primary-text` 신설, 사용처 3곳 동시 해소
- [x] **LCP 708ms** (모바일 Slow 4G·CPU 4x, 예산 2.5초의 28%) · CLS 0.00
- [x] **Vercel 플랜 판단 — Hobby 유지** (무료·무광고·무후원이라 상업 조항 비해당. 월 100GB 대비 실측 300KB/방문 = 30만 방문 상당 여유)
- [x] 🟡 **iOS Safari 순회 — 부분 검증** — 실기기 수단 미확보로 **Mac Safari 26.6.2(동일 WebKit)** 대체. 엔진 기능 8/8 실측 통과(`DecompressionStream("deflate-raw")` 폴백 부재가 최대 블로커였다) + 3라우트 전수 + iPhone 프리셋 세로/가로 정상
- [ ] ⚠️ **iOS 고유 정책 2건 미검증** — `navigator.share` transient activation · 주소창 축소/safe-area. 미검증을 명시하고 런칭 확정(사용자 결정)
- [ ] ⚠️ **접근성 이월 1건 미처리** — 비트 디밍 구간 대비 **2.34:1**(Day 11에서 이 게이트로 이관됨). 이 게이트는 **다른 항목**(링크 대비)을 고치고 통과 처리됐고 이 건은 손대지 않았다. 2026-09-09 발견. `aria-hidden`이고 같은 값이 1.2초 뒤 카드에서 19.76:1로 재노출되므로 **유지 결정도 가능**하나, 결정으로 기록돼야 한다 → 사용자 판단 대기

### 사용자 담당

- [x] ~~실기기 수단 확보 (Day 16 밤 마감)~~ — **실패**(Xcode 미설치·실기기 미섭외). Mac Safari 대체로 종결
- [ ] 🔴 **비트 디밍 대비 판단** — 0.35 → 0.5 완화 vs 현행 유지. 연출 강도와 맞바꾸는 결정이라 사용자 몫

## Day 18~19 (**9/4~9/5**) — 런칭 포스트 준비

### Claude 담당

- [x] **채널 선정 4 → 3** — GeekNews · 디스콰이엇 · Threads. 축소분의 위험은 표본 구성으로 이전됨을 명시
- [x] **런칭 포스트 3종 작성 → 승인·문구 동결** (2026-09-04)
- [x] **전 링크 UTM** — 6링크 누락 0 확인
- [x] **당일 값 최종 치환** (2026-09-08) — `B+ 50점 · 한 줄 사령관 · Bash 138회`, 16:19 스냅샷으로 동결
- [x] 인터뷰 가이드 작성 (`docs/인터뷰.md`) — 이후 발동 조건부 항목으로 이관
- [x] 🐛 **이슈 #1 원인 규명** — 카카오톡이 `#` 프래그먼트 URL을 스크랩하지 않음. 단일 변수 테스트로 인과 확정
- [x] ~~GIF 헤드 자산~~ → **폐지 2026-09-08**(사용자 결정). 대본은 `docs/런칭포스트.md`에 보존, 재설계 비용 0

### 사용자 담당

- [x] 런칭 포스트 3종 승인 (2026-09-04)
- [x] 채널 3개 확정 · GIF 헤드 자산 폐지 결정

## Day 20 (**9/10 목**) — 런칭

> 9/8~9/9는 **확장 프로토콜 1번(QA 심화)** 구간. 신기능·backlog 부활 금지.

### Claude 담당 — 사전 검증·정비 (9/8~9/9 완료분)

- [x] **프로덕션 사전 검증** (2026-09-08) — 5라우트 200 · 동적 OG 28,701B · 스크래퍼 UA 4종 · UTM 6링크 누락 0 · npm `0.1.1`
- [x] **6차 일정 조정 반영** — 런칭 9/8 → 9/10, 판정일 9/24. 스코프 축소 2건(GIF·인터뷰) 기록
- [x] **보안 응답 헤더 미적용 결정** + CSP `connect-src` backlog 등재 (근거: 효과가 측정되지 않음 / CSP는 1줄 작업 아님)
- [x] **`llms.txt` backlog 기재 정정** — "미구현"이 아니라 "있는 파일이 왜 감사에 걸리는가"
- [x] **Day 18·20 EOD Notion 11건 등록** (2026-09-09) — 5일 밀려 있던 Day 18분 5건 포함. Obsidian TIL 138건
- [x] 🆕 **브랜드 자산 미이행 회수** (2026-09-09) — `create-next-app` 기본 파비콘 교체 + `icon`·`apple-icon` 신설 + 스캐폴드 SVG 5개 삭제 + 워드마크 2벌 보관. **프로덕션 반영 확인**(head 태그 3종·5라우트 200·삭제분 404)
- [x] **TODO.md 동기화** (2026-09-09) — 이 문서. Day 13~20 사후 재구성
- [ ] 게시 직전 최종 점검 — 5라우트 · 동적 OG · UTM · npm 버전 재확인
- [ ] 이슈 #1 처리 방침 기재 — OPEN 상태로 런칭을 통과함을 이슈에 명시
- [ ] 게시 후 Sentry 신규 에러 감시 · GA4 실발화 확인

### 사용자 담당

- [ ] 🔴 **9/8~9/9 전 플로우 완주** — 문제점·변경 희망사항 회수. **자문 ①②④ 회수를 여기에 얹을 것**(마지막 기회)
- [ ] 새 브랜드 아이콘 육안 확인 — 탭 · 북마크 · iOS 홈 화면 추가
- [ ] 🔴 **채널 2분 테스트** — 카드 렌더 확인 (Day 18 신설 절차)
- [ ] 🔴 **게시** — GeekNews → 디스콰이엇 → Threads(첫 댓글 링크). 계정이 본인이라 대행 불가

## Day 21 (**9/11 금**) — 버퍼 + 초기 반응 대응

### Claude 담당

- [ ] Sentry 신규 에러 조회·분류 (치명만 수정 대상)
- [ ] 치명 버그 수정 — 새 브랜치 → 테스트 → main 머지 → 배포 확인
- [ ] GA4 첫날 지표 판독 — 도달률·공유/저장률만. **일별 등락 해석 금지**
- [ ] 채널 반응 수집·분류 + 답변 초안
- [ ] 요청 사항 `backlog.md` 이관 (구현 금지, 등재만)
- [ ] Day 21 마일스톤 기록 — 전체 회고 + 초기 지표
- [ ] 운영 루틴·판정일 캘린더 등록 (상시 규칙: 게이트·마감의 캘린더 등록)
- [ ] 9/24 판정 대조표 골격 준비 (숫자 기입은 당일)

### 사용자 담당

- [ ] 커뮤니티 댓글 직접 응답 (30분 상한)
- [ ] 치명 vs backlog 최종 판정
- [ ] GA4 확인 요일 2개 확정 (주 2회 고정)
- [ ] ~~초기 유저 인터뷰 섭외~~ — **폐지 2026-09-08**. 발동 조건은 판정일 지표 미달 시 "피벗" 선택하려는 경우뿐

## 런칭 후 ~ 판정일 (9/11~9/24)

- [ ] 매일 30분 상한 — 댓글 응답 · Sentry 확인 · 치명 버그만 수정
- [ ] 주 2회 GA4 (요일 고정) — **재방문율·공유율만** 본다
- [ ] EOD 기록 격일 유지
- [ ] 🚫 **판정 전 신기능 개발 금지** (컷된 재미 장치 부활 포함 — 판정을 오염시킨다)
- [ ] 📅 **9/24(목) 킬 크라이테리아 판정** — PLAN §판정 해석 규칙 적용(지속 / 인터뷰 후 피벗 / 중단). 외부 검증 0이라 **숫자만으로는 "피벗"을 고를 수 없다**
