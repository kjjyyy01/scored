...
- [ ] **EOD 기록 세션 (8/26 저녁, 별도 세션)** — 아래 인계 참조
  - ✅ **선행 완료**: Obsidian TIL 정정 1건 — `TIL-카카오톡은-초장문-URL을-스크랩하지-않는다.md` **삭제** → `TIL-카카오톡은-프래그먼트-포함-URL을-스크랩하지-않는다.md` 신규 작성 (백링크 0건 확인 후 교체). **재작업 금지**
  - 남은 Obsidian: 신규 TIL 3건 — ① 임계값 가설은 경계 두 점으로만 반증된다(81 vs 102자) ② 깔때기 분모에 도달 경로가 없으면 지표가 아니라 제품이 안 보인다(EL-RPT-008) ③ 한국어 `word-break:keep-all`은 `overflow-wrap:break-word`와 짝이다. + 원고 백업 `_notion-원고-scored-Day9.md`
  - 남은 Notion 「개발 로그:scored」(DB `d354f12e-593d-498d-bf9f-afa10c8dfbce` / DS `collection://d3cc1b91-0b5a-40a2-a215-888cbff243b3`): **TS-26 정정 필수** (페이지 `3c7203a3-d74e-81e2-8f11-e996ce0d9232`, 제목의 "범인은 URL 길이"가 반증됨) + 과정요약 1 + 트러블슈팅 TS-27~29 3건. 누적 31건 → 35건 예정
  - ⚠️ Obsidian `dev-notes/`는 **scored·stackd 공용** — 원고 파일명 접두사(`_notion-원고-scored-`) 확인할 것. stackd Day 9 원고는 8/26 22:00에 이미 별도 세션에서 작성됨
