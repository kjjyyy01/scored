...
- [ ] 라이트모드 — **이미 동작**(`:root` 라이트 기본 + `prefers-color-scheme: dark` 오버라이드, DESIGN.md 2026-08-18). 대비 실측: 빌드업 4.79:1 · 릴/행 19.76:1 통과, **비트 디밍 구간만 2.34:1 미달**(라이트·다크 공통, Day 17 접근성 스팟 대상). 잔여 결정: OS 무관 **토글 UI**를 원하는지 여부 — DESIGN.md는 v2 backlog로 미뤄둔 항목
